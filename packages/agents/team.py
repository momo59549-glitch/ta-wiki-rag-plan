...
from packages.research import FileResearchPipeline, PipelineConfig, build_experiment_protocol, verify_code_snapshot
...
class FileResearchTeam:
    ...
    @staticmethod
    def _validated_campaign(campaign_dir: Path, symbols: list[str], rule: CompiledRule, config: TeamConfig) -> dict:
        ...
        data_check = verify_strong_snapshot(campaign_dir / names["dataset"])
        ...
        checks = {
            ...
            "dataset_bound": protocol.get("dataset_snapshot_id") == data_check.get("dataset_snapshot_id"),
        ...
        return {**payload, "directory": campaign_dir, "checks": checks}

    def run(self, symbols: list[str], rule: CompiledRule, config: TeamConfig, *, frozen_campaign: Path | None = None) -> Path:
        frozen = self._validated_campaign(frozen_campaign, symbols, rule, config) if frozen_campaign else None
        case_id = "case_" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ") + "_" + uuid4().hex[:8]
        case_dir = self.output_root / case_id
        lifecycle = FileCaseStateMachine.create(case_dir, case_id)
        ...
        lifecycle.transition(CaseState.OBSERVATIONS_READY, "observations.ready", {"count": len(observations)})
        lifecycle.transition(CaseState.OUTCOMES_READY, "outcomes.ready", {"count": len(outcomes)})

        research = self._research(outcomes, config, statistics)
        ...
        lifecycle.transition(CaseState.BACKTEST_REVIEWED, "backtest.reviewed", {"outcomes_checked": len(outcomes)})
        ...
        knowledge = {
            ...
            "evidence_refs": [],
            "research_artifacts": [f"{run_path}/observations.jsonl", f"{run_path}/outcomes.jsonl", "statistics_out_of_sample.json"],
    ...
    @staticmethod
    def _research(outcomes: list[dict], config: TeamConfig, statistics: dict) -> dict:
    ...
    def _qa(case_dir: Path, run_dir: Path, run: dict, outcomes: list[dict], research: dict, config: TeamConfig) -> dict:
        required = [run_dir / name for name in ("config.json", "run.json", "progress.json", "observations.jsonl", "outcomes.jsonl", "report.md")]
