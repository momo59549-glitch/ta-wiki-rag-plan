...
from packages.market_data import load_point_in_time_universe, load_universe_memberships, verify_strong_snapshot
...
def build_code_snapshot(project_root: Path, output: Path) -> dict[str, Any]:
    ...
    files = [{"path": path.relative_to(project_root).as_posix(), "size": path.stat().st_size, "sha256": _sha256(path)} for path in sorted(set(candidates))]
